...
params = get_params()
best_reward = -np.inf
if os.path.exists(os.path.join(DIR_CHECKPOINTS, 'acrobot-params.npy')):
    params = np.load(os.path.join(DIR_CHECKPOINTS, 'acrobot-params.npy'))
    best_reward = np.load(os.path.join(DIR_CHECKPOINTS, 'acrobot-best.npy'))
    print('Previous best')
    sample(5)
    exit()
    print('Now training')
...
while best_reward / episodes < 0:
    ...
    if epoch_reward > best_reward:
        ...
        np.save(os.path.join(DIR_CHECKPOINTS, 'acrobot-params'), params)
        np.save(os.path.join(DIR_CHECKPOINTS, 'acrobot-best'), best_reward)
        sample()
        std = 100