...
params = get_params()
best_reward = -np.inf
if os.path.exists(os.path.join(DIR_CHECKPOINTS, 'mountaincar-params.npy')):
    params = np.load(os.path.join(DIR_CHECKPOINTS, 'mountaincar-params.npy'))
    best_reward = np.load(os.path.join(DIR_CHECKPOINTS, 'mountaincar-best.npy'))
    print('Previous best')
    sample(10)
    exit()
    print('Now training')
...
while best_reward / episodes < 0:
    ...
    if epoch_reward > best_reward:
        ...
        np.save(os.path.join(DIR_CHECKPOINTS, 'mountaincar-params'), params)
        np.save(os.path.join(DIR_CHECKPOINTS, 'mountaincar-best'), best_reward)
        #sample()
        std = 100